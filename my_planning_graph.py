...
class PlanningGraph:
    ...
    def create_graph(self):
        ...
        if (len(self.s_levels) != 0) or (len(self.a_levels) != 0):
            raise Exception('Planning Graph already created; construct a new planning graph for each new ' +
                            'state in the planning sequence')
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if act.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(act)

                for state in act.prenodes:
                    state.children.add(act)
                    act.parents.add(state)
    ...
    def add_literal_level(self, level):
        ...
        #   create and connect all of the new S nodes as children of all the A nodes that could produce them,
        #   and likewise add the A nodes to the parent sets of the S nodes

        self.s_levels.append(set())

        for act in self.a_levels[level - 1]:
            for state in act.effnodes:
                self.s_levels[level].add(state)
                state.parents.add(act)
                act.children.add(state)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        add1 = set(node_a1.action.effect_add)
        add2 = set(node_a2.action.effect_add)
        rem1 = set(node_a1.action.effect_rem)
        rem2 = set(node_a2.action.effect_rem)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        s1 = set(node_a1.action.effect_add)
        s2 = set(node_a2.action.precond_neg)

        if s1 & s2:
            return True

        s1 = set(node_a1.action.effect_rem)
        s2 = set(node_a2.action.precond_pos)

        if s1 & s2:
            return True

        s1 = set(node_a2.action.effect_add)
        s2 = set(node_a1.action.precond_neg)

        if s1 & s2:
            return True

        s1 = set(node_a2.action.effect_rem)
        s2 = set(node_a1.action.precond_pos)
    ...
    def h_levelsum(self) -> int:
        ...
        enumerated_states = enumerate(self.s_levels)

        for goal in self.problem.goal:
            state = PgNode_s(goal, True)

            for i, states in enumerated_states:
                if state in states:
                    level_sum += i
                    break
